...
// Soft-delete a visitor's own recent message.
...
enforce_rate_limit($pdo, 'widget_message_delete', rate_limit_identifier($siteKey . '|' . $visitorId), 20, 10 * 60);
...
try {
    ...
    if (!message_can_be_modified_by($message, 'visitor', $visitorId)) {
        json_response(['success' => false, 'message' => 'This message can no longer be deleted'], 403);
    }

    $pdo->beginTransaction();
    $revisionStmt = $pdo->prepare("\n        INSERT INTO message_revisions (message_id, editor_type, editor_id, action, previous_content, new_content)\n        VALUES (:message_id, 'visitor', :editor_id, 'delete', :previous_content, NULL)\n    ");
    ...
    $deleteStmt = $pdo->prepare("\n        UPDATE messages\n        SET content = 'این پیام حذف شده است.', deleted_at = NOW(), deleted_by_type = 'visitor', deleted_by_id = :editor_id\n        WHERE id = :message_id AND deleted_at IS NULL\n    ");
    $deleteStmt->execute([':editor_id' => $visitorId, ':message_id' => $messageId]);
    $pdo->commit();
