...
try{$pdo->beginTransaction();
 if($action==='confirm'){
  ...
  if($secret===''||!totp_verify($secret,$code))throw new ApiPublicException('کد Authenticator صحیح نیست.');
  ...
  $pdo->commit();auth_clear_session_cookies(auth_request_uses_impersonation());json_response(['success'=>true,'message'=>'ورود دومرحله‌ای فعال شد. دوباره وارد شوید.','recovery_codes'=>$codes,'requires_relogin'=>true]);
 }
 if($action==='disable'){
