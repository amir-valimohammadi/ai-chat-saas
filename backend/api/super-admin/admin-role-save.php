...
$placeholders=implode(',',array_fill(0,count($codes),'?'));$q=$pdo->prepare("SELECT id,code FROM admin_permissions WHERE code IN ($placeholders)");$q->execute($codes);$valid=$q->fetchAll();if(count($valid)!==count($codes))throw new ApiPublicException('یک یا چند مجوز نامعتبر است.');
if($id===0){$base=preg_replace('/[^a-z0-9]+/','-',strtolower(trim((string)($in['code']??''))));$base=trim($base,'-');if($base===''||strlen($base)>70)throw new ApiPublicException('کد انگلیسی نقش معتبر نیست.');$exists=$pdo->prepare('SELECT COUNT(*) FROM admin_roles WHERE code=:code');$exists->execute([':code'=>$base]);if((int)$exists->fetchColumn()>0)throw new ApiPublicException('کد نقش تکراری است.');
...
}else{$oldStmt=$pdo->prepare('SELECT id,code,name,description,is_system,is_active FROM admin_roles WHERE id=:id LIMIT 1 FOR UPDATE');$oldStmt->execute([':id'=>$id]);$old=$oldStmt->fetch();if(!$old)throw new ApiPublicException('نقش پیدا نشد.');if($old['code']==='owner')throw new ApiPublicException('مجوزهای نقش مالک قابل تغییر نیست.');if((int)$old['is_system']===1&&!$actor['is_platform_owner'])throw new ApiPublicException('فقط مالک پلتفرم می‌تواند نقش سیستمی را تغییر دهد.');$pdo->prepare('UPDATE admin_roles SET name=:name,description=:description,updated_at=NOW() WHERE id=:id')->execute([':name'=>$name,':description'=>$description!==''?$description:null,':id'=>$id]);}
...
}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();json_response(['success'=>false,'message'=>safe_api_exception_message($e,'ذخیره نقش مدیریتی ناموفق بود.')],422);}
