...
if(in_array($profile,['quick','full','security','security_deep','operational','browser'],true)){ $where[]='profile=:profile';$params[':profile']=$profile; }
...
try{
    ...
    $stmt=$pdo->prepare("SELECT id,run_key,profile,target_type,target_id,target_label,status,total_count,passed_count,warning_count,failed_count,skipped_count,score_percent,duration_ms,progress_percent,current_case_key,heartbeat_at,cancel_requested_at,environment,reason,triggered_by,triggered_by_name,started_at,finished_at,error_message,created_at FROM qa_test_runs {$sqlWhere} ORDER BY id DESC LIMIT {$perPage} OFFSET {$offset}");$stmt->execute($params);
...
}catch(Throwable $e){json_response(['success'=>false,'message'=>'دریافت تاریخچه تست ناموفق بود.','request_id'=>defined('APP_REQUEST_ID')?APP_REQUEST_ID:null],500);}
