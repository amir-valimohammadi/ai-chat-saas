...
try{
    ...
    $permission=$source['profile']==='security'?'tests.run_security':($source['profile']==='operational'?'tests.run_operational':($source['profile']==='full'?'tests.run_full':'tests.run_safe'));require_admin_permission($user,$permission);
    if(in_array($source['profile'],['security','operational'],true))require_sensitive_confirmation($pdo,$user,$input);
    ...
    $runId=(int)$pdo->lastInsertId();$run=qa_execute_run($pdo,$runId,$keys);admin_audit_log($pdo,$user,'qa_test_failed_rerun','qa_test_run',$runId,'موارد ناموفق تست دوباره اجرا شدند.',null,['source_run_id'=>$sourceId,'case_count'=>count($keys)]);
    json_response(['success'=>true,'message'=>'موارد ناموفق دوباره اجرا شدند.','run_id'=>$runId,'run'=>$run]);
}catch(Throwable $e){json_response(['success'=>false,'message'=>'اجرای مجدد تست‌ها ناموفق بود.','error'=>$e->getMessage()],500);}
