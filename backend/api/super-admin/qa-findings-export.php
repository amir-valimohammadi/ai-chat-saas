...
$format=strtolower(trim((string)($_GET['format']??'csv')));$scope=trim((string)($_GET['scope']??'open'));$runId=max(0,(int)($_GET['run_id']??0));
...
try{
    ...
    if($format==='json'){
        ...
        echo json_encode(['generated_at'=>date('c'),'scope'=>$scope,'run_id'=>$runId?:null,'total'=>count($normalized),'findings'=>$normalized],JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);exit;
    }
    ...
    fputcsv($out,['شناسه','کلید تست','دسته','عنوان','هدف','وضعیت ثبت','نتیجه تست','شدت','شرح مشکل','دلیل احتمالی','اثر و ریسک','مقدار مورد انتظار','مقدار فعلی','راهکار رفع','شواهد فنی','تعداد تکرار','اولین مشاهده','آخرین مشاهده','آخرین Run','یادداشت حل']);
    foreach($normalized as $row){
        fputcsv($out,[$row['id'],$row['case_key'],$row['category'],$row['title'],$row['target_label']??$row['target_type'],$row['registry_status'],$row['test_status'],$row['severity'],$row['message'],$row['root_cause'],$row['impact'],$row['expected_value'],$row['actual_value'],$row['remediation'],json_encode($row['evidence'],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$row['occurrence_count'],$row['first_seen_at'],$row['last_seen_at'],$row['last_run_id'],$row['resolution_note']]);
    }
    fclose($out);exit;
}catch(Throwable $e){json_response(['success'=>false,'message'=>'ساخت خروجی ایرادات ناموفق بود.','error'=>$e->getMessage()],500);}
