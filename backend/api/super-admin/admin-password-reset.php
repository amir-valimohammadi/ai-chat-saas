...
try{$pdo->beginTransaction();$q=$pdo->prepare("SELECT id,name,email FROM users WHERE id=:id AND role='super_admin' LIMIT 1 FOR UPDATE");$q->execute([':id'=>$id]);$target=$q->fetch();if(!$target)throw new ApiPublicException('مدیر پیدا نشد.');
...
$pdo->commit();json_response(['success'=>true,'message'=>'رمز مدیر بازنشانی شد.']);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();json_response(['success'=>false,'message'=>safe_api_exception_message($e,'بازنشانی رمز مدیر ناموفق بود.')],422);}
