...
export default function TestRunsHistoryPage(){
    ...
    return <AppShell title="تاریخچه تست‌ها"><main className="qa-center-page">
        ...
        <section className="qa-card"><div className="qa-filter-bar qa-history-filter"><select value={profile} onChange={e=>setProfile(e.target.value)}><option value="all">همه پروفایل‌ها</option><option value="quick">تست سریع</option><option value="full">تست کامل</option><option value="operational">تست عملیاتی</option><option value="security">تست امنیتی</option></select><select value={status} onChange={e=>setStatus(e.target.value)}><option value="all">همه وضعیت‌ها</option><option value="completed">تکمیل‌شده</option><option value="running">در حال اجرا</option><option value="failed">خطای Runner</option><option value="cancelled">لغوشده</option></select><span>{new Intl.NumberFormat("fa-IR").format(pagination.total)} اجرا</span></div></section>
...
function profileLabel(value:Run["profile"]){return value==="quick"?"سریع":value==="full"?"کامل":value==="operational"?"عملیاتی":"امنیتی"}
